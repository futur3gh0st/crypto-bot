...
class BookPaperEngine:
    ...
    def step(self, live: LiveBookSignals, decision: RiskDecision) -> list[BookPaperFill]:
        ...
        for funding_row in live.funding:
            if funding_row.error or not funding_row.enter:
                continue
            if funding_row.inst_id in self.state["funding"]:
                fills.append(BookPaperFill("funding", funding_row.inst_id, 0, 0, 0, True, "already in"))
                continue
            room = max(0.0, sleeve_f - used_f)
            notional = min(self.equity * self.cfg.funding.equity_frac, room)
            if notional < 10:
                fills.append(BookPaperFill("funding", funding_row.inst_id, 0, 0, 0, True, "no sleeve room"))
                continue
            fee = cost_usd(notional, ow)
            self.state["funding"][funding_row.inst_id] = {
                "notional": notional,
                "side": 1 if (funding_row.trail_avg or 0) > 0 else -1,
            }
            used_f += notional
            self.equity -= fee
            opp = self._dummy_opp("funding", funding_row.inst_id)
            self.ledger.record_fill(opp, notional, -fee, fee, 1.0, "funding enter (paper)")
            fills.append(BookPaperFill("funding", funding_row.inst_id, notional, -fee, fee, False, "enter"))
        ...
        if not decision.trade:
            ...
            if size_mult < 1:
                reason = decision.reason
        for depeg_row in live.depegs:
            if not depeg_row.enter or depeg_row.mid is None:
                continue
            if depeg_row.asset in self.state["depeg"]:
                fills.append(BookPaperFill("depeg", depeg_row.pair, 0, 0, 0, True, "already in (no restack)"))
                continue
            if skip:
                fills.append(BookPaperFill("depeg", depeg_row.pair, 0, 0, 0, True, reason))
                continue
            ...
            if room < 10:
                fills.append(BookPaperFill("depeg", depeg_row.pair, 0, 0, 0, True, "no sleeve / fear cut to ~0"))
                continue
            fee_bps = self.cfg.fee_bps(depeg_row.venue) + self.cfg.strategy.hist_half_spread_bps
            fee = room * fee_bps / 10_000.0
            self.state["depeg"][depeg_row.asset] = {"notional": room, "px": depeg_row.mid, "pair": depeg_row.pair}
            used_d += room
            self.equity -= fee
            opp = self._dummy_opp("depeg_fade", depeg_row.pair)
            self.ledger.record_fill(opp, room, -fee, fee, size_mult, reason)
            fills.append(BookPaperFill("depeg", depeg_row.pair, room, -fee, fee, False, reason))
        self._save()
        return fills
